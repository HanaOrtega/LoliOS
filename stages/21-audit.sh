...
audit_grep '^defaultFileSystemType:[[:space:]]*btrfs' "$PROFILE/airootfs/etc/calamares/modules/partition.conf" "Calamares default filesystem type is Btrfs"
audit_grep '^defaultFileSystem:[[:space:]]*btrfs' "$PROFILE/airootfs/etc/calamares/modules/partition.conf" "Calamares default filesystem is Btrfs"
audit_grep '^[[:space:]]*- btrfs' "$PROFILE/airootfs/etc/calamares/modules/partition.conf" "Calamares exposes Btrfs filesystem choice"
audit_grep 'compress=zstd:3' "$PROFILE/airootfs/etc/calamares/modules/partition.conf" "Calamares Btrfs mount options enable zstd compression"
audit_grep 'space_cache=v2' "$PROFILE/airootfs/etc/calamares/modules/partition.conf" "Calamares Btrfs mount options enable space_cache v2"
